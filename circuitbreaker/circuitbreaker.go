...
func (cb *CircuitBreaker) State() State {
	return cb.beforeCall().state
}

// Guard returns a Guard for if-like flat workflow,
// throw it away after it has been used. You can use Run for "convenience".
func (cb *CircuitBreaker) Guard() *Guard {
	state := cb.beforeCall()
...
// Guard guards the workflow, throw away after it has been used.
type Guard struct {
	state   *internalState
...
// true means any normal event may happen at time now.
// Allow always returns false after being used.
...
// Observe does nothing after being used.
func (g *Guard) Observe(startAt time.Time, statusOk bool) {
	if !g.allow || g.invalid2 { // Only allowed call can be observed.
		return
	}
	g.invalid2 = true
