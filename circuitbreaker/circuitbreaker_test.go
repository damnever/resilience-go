...
func TestGuard(t *testing.T) {
	...
	g.Observe(time.Now(), true)

	// Check if any call passed.
	g.state = nil
	g.breaker = nil
	for i := 0; i < 100; i++ {
...
func TestCircuitBreaker(t *testing.T) {
	...
	testconf := Config{
		...
		SlowCallRateThreshold:     0.5,
		SlowCallDurationThreshold: 10 * time.Second,
		OnStateChange: func(pre, current State) {
			t.Logf("state: %s -> %s", pre, current)
		},
	}
	cb := New(testconf)
	...
	wg.Wait()
	require.Equal(t, int32(testconf.AllowedProbeCalls), probes)
